...
TEST_SERVER_LOGGER = logging.getLogger('test_server')
...
def process_client_message(message):
    ...
    if ACTION in message and message[ACTION] == PRESENCE and TIME in message and \
        ...
        SERVER_LOGGER.debug(f'Формирование {PRESENCE} от {message[USER][ACCOUNT_NAME]} успешно')
        return {RESPONSE: 200}
    SERVER_LOGGER.debug(f'Формирование {PRESENCE} от {message[USER][ACCOUNT_NAME]} провалено')
    return {
        RESPONSE: 400,
        ERROR: 'Bad Request'
    }
...
def create_arg_parser():
    ...
    :return:
    """
    TEST_SERVER_LOGGER.debug('Функция парсера отработала')
    parser = argparse.ArgumentParser()
    parser.add_argument('-p', default=DEFAULT_PORT, type=int, nargs='?')
    parser.add_argument('-a', default='', nargs='?')
    TEST_SERVER_LOGGER.debug(f'Данные парсера: {parser}')
    return parser
